/*
Page 354
Algorithm 6.9 Pseusocode to compute dfn and L
Algorithm Art(u,v)
//u is a start vertex for depth first search. V is its parent if any
//in the depth first spanning tree.It is assumed that the global array
//dfn is initialized to 1. n is the number of vertices in G
{
    dfn[u] = num;
    L[u] = num;
    num = num + 1;

    for each vertext w adjacent from u do
    {
        if(dfn[w] == 0)
        {
